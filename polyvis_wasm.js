 ...
 * @returns {string}
 */
export function serialize_session(session, format) {
    let deferred2_0;
    let deferred2_1;
    try {
        _assertClass(session, Instance);
        const ret = wasm.serialize_session(session.__wbg_ptr, format);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
export class Instance {
     ...
     * @returns {bigint}
     */
    add_polygon(points, color, selected) {
        const ptr0 = passArrayF64ToWasm0(points, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.instance_add_polygon(this.__wbg_ptr, ptr0, len0, color, selected);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
